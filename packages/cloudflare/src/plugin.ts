...
export const cloudflarePlugin = (
  ...
  capabilities: (host: HostServicesShape) => {
    ...
    }).pipe(Effect.withSpan("CloudflarePlugin.sync"), fsError("sync failed"))

    /** Merge the plugin's dev-only secret values into `.dev.vars`, preserving hand-written keys. */
    const mergeDevVars = Effect.fn("CloudflarePlugin.mergeDevVars")(function* (
      values: Readonly<Record<string, string>>,
    ) {
      const varsPath = yield* pathOf(".dev.vars")
      const existing = yield* fs.readFileString(varsPath).pipe(Effect.orElseSucceed(() => ""))
    ...
    const build = Effect.gen(function* () {
      yield* sync
      yield* runVite(["build"])
    }).pipe(Effect.withSpan("CloudflarePlugin.build"))

    const preview = runVite(["preview"]).pipe(Effect.withSpan("CloudflarePlugin.preview"))

    const seed = Effect.fn("CloudflarePlugin.seed")(function* (opts: { url: string }) {
      const projectRoot = yield* ProjectRoot
